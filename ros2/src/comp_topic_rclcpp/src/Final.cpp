...
public:
    Final() : Node("Final")
    {
        ...
        // 初始化日志文件，覆盖旧数据
        initialize_log_file("LaneNode");
        initialize_log_file("YoloNode");
        initialize_log_file("TrackerNode");
...
private:
    ...
    void initialize_log_file(const std::string &topic_name)
    {
        ...
        std::string log_file = log_path + "/tra_FinalNode_" + topic_name;
    ...
    void lane_result_callback(const comp_topic_rclcpp::msg::LaneResult::SharedPtr msg)
    {
        ...
        // 记录传输延迟
        log_tra_latency("LaneNode", msg->header.stamp);
    ...
    void tracker_result_callback(const comp_topic_rclcpp::msg::TrackerResult::SharedPtr msg)
    {
        ...
        // 记录传输延迟
        log_tra_latency("TrackerNode", msg->header.stamp);
