...
public:
    Tracker() : Node("Tracker"), tracking_initialized_(false), yolo_result_received_(false)
    {
        ...
        // 初始化日志文件
        initialize_log_file("tra_TrackerNode_Frame");
        initialize_log_file("tra_TrackerNode_YoloNode");
        initialize_log_file("cal_TrackerNode");
    }
...
private:
    ...
    {
        std::string log_file = log_path + "/" + log_name;
    ...
    void log_tra_latency(const std::string &topic_name, const rclcpp::Time &msg_time)
    {
        ...
        std::string log_file = log_path + "/tra_TrackerNode_" + topic_name;
    ...
    {
        std::string log_file = log_path + "/cal_TrackerNode";
    ...
    {
        RCLCPP_INFO(this->get_logger(), "Tracker rcvd Frame");

        // 记录消息传输延迟
        log_tra_latency("Frame", msg->header.stamp);
    ...
    {
        RCLCPP_INFO(this->get_logger(), "Tracker rcvd yolo");

        // 记录消息传输延迟
        log_tra_latency("YoloNode", msg->header.stamp);
