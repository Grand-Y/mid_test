...
public:
    Yolo(std::string name) : Node(name), env(ORT_LOGGING_LEVEL_WARNING, "YOLOv5")
    {
        ...
        // 初始化日志文件
        initialize_log_file("tra_YoloNode_Frame");
        initialize_log_file("cal_YoloNode");
...
private:
    ...
    void log_tra_latency(const rclcpp::Time &msg_time)
    {
        ...
        std::string log_file = log_path + "/tra_YoloNode_Frame";
    ...
    {
        std::string log_file = log_path + "/cal_YoloNode";
    ...
    void cameratimer_topic_callback(const sensor_msgs::msg::Image::SharedPtr msg)
    {
        ...
        RCLCPP_INFO(this->get_logger(), "Yolo send res");
