...
bool comp_topic_rclcpp__msg__tracker_result__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  ...
  {  // width
    ...
    if (PyObject_CheckBuffer(field)) {
      ...
      double * dest = ros_message->width.data;
      rc = PyBuffer_ToContiguous(dest, &view, view.len, 'C');
      if (rc < 0) {
        PyBuffer_Release(&view);
        Py_DECREF(field);
  ...
  {  // height
    ...
    if (PyObject_CheckBuffer(field)) {
      ...
      double * dest = ros_message->height.data;
      rc = PyBuffer_ToContiguous(dest, &view, view.len, 'C');
      if (rc < 0) {
        PyBuffer_Release(&view);
        Py_DECREF(field);
...
PyObject * comp_topic_rclcpp__msg__tracker_result__convert_to_py(void * raw_ros_message)
{
  ...
  {  // width
    ...
    if (ros_message->width.size > 0) {
      ...
      PyObject * frombytes = PyObject_GetAttrString(field, "frombytes");
      assert(frombytes != NULL);
      double * src = &(ros_message->width.data[0]);
      PyObject * data = PyBytes_FromStringAndSize((const char *)src, ros_message->width.size * sizeof(double));
      assert(data != NULL);
      PyObject * ret = PyObject_CallFunctionObjArgs(frombytes, data, NULL);
      Py_DECREF(data);
      Py_DECREF(frombytes);
  ...
  {  // height
    ...
    if (ros_message->height.size > 0) {
      ...
      PyObject * frombytes = PyObject_GetAttrString(field, "frombytes");
      assert(frombytes != NULL);
      double * src = &(ros_message->height.data[0]);
      PyObject * data = PyBytes_FromStringAndSize((const char *)src, ros_message->height.size * sizeof(double));
      assert(data != NULL);
      PyObject * ret = PyObject_CallFunctionObjArgs(frombytes, data, NULL);
      Py_DECREF(data);
      Py_DECREF(frombytes);
